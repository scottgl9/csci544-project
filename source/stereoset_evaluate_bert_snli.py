...
if __name__ == '__main__':
    ...
    # Load the SNLI fine-tuned model
    # We'll use the 'textattack/bert-base-uncased-snli' model for this purpose
    # However, since it's a sequence classification model, we'll extract its encoder weights
    snli_model_name = 'textattack/bert-base-uncased-SNLI'
    snli_model = transformers.BertForSequenceClassification.from_pretrained(snli_model_name)

    # Load the tokenizer
    tokenizer = transformers.BertTokenizer.from_pretrained('bert-base-uncased', padding_side='right')

    # Initialize a BertForMaskedLM model with the encoder weights from the SNLI fine-tuned model
    model = transformers.BertForMaskedLM.from_pretrained('bert-base-uncased')
    model.bert = snli_model.bert  # Replace the encoder weights with SNLI fine-tuned weights
    model.to(device)
    model.eval()

    evaluate_stereoset_intrasentence(model, tokenizer=tokenizer, device=device)
